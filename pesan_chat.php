...
$role = 'konselor';  // Role pengguna

// Simpan pesan teks
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['pesan'])) {
    $pesan = trim($_POST['pesan']);
    $penerima = $_GET['user'] ?? ''; // Pastikan validasi di produksi
...
");
$stmt->bind_param("ss", $user_yang_dipilih, $user_yang_dipilih);
$stmt->execute();
...
<div class="content">
    <div class="chat-box">
        <div class="chat-header"><?= htmlspecialchars($user_yang_dipilih) ?></div>

        <div class="chat-messages" id="chat-messages">
            <?php while ($row = $pesan_result->fetch_assoc()): ?>
             <div class="chat-bubble <?= $row['pengirim_role'] === 'user' ? 'konselor' : 'user' ?>">
